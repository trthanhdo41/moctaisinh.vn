...
class CartGuard {
    ...
    protectCheckoutPage() {
        // Kiểm tra thông tin đơn hàng từ URL
        ...
        if (!orderType || !orderId) {
            // Không có thông tin đơn hàng hợp lệ, chuyển hướng về trang sản phẩm
            this.showInvalidOrderError();
            return false;
        }
        
        // Kiểm tra dựa trên loại đơn hàng
        if (orderType === 'buynow') {
            // Đơn hàng "Mua ngay"
    ...
    replacePageContent(html) {
        // Thay thế toàn bộ nội dung trang
    ...
    // Phương thức tiện ích để kiểm tra từ bên ngoài
...
// Khởi tạo CartGuard khi trang load
document.addEventListener('DOMContentLoaded', function() {
    new CartGuard();
});

// Export để sử dụng từ các file khác
